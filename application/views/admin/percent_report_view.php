                        ...
                        <div class="form-group">
                            ...
                            <div class="input-icon ">
                                <select class="form-control select2me " id="test_name" name="test_name" required
                                    style="width: 30%; border: none !important; border-radius: 0% !important;" onchange="filterExam()">
                                    <option value="">Select Test</option>
                                    <?php if (!empty($exam_list)) {
                                        foreach ($exam_list as $key) { ?>
                                            <option value="<?php echo $key->test_configuration_id; ?>" <?= (!empty($test_id) && $test_id == $key->test_configuration_id) ? 'selected' : ''; ?>>
                        ...
                        <div class="row form">
                            <div class="portlet box blue-hoki">
                                <div class="portlet-title">
                                    <div class="caption">
                                        <i class="fa fa-list"></i>Correct vs Wrong % Report
                                    </div>
                                </div>
                                <div class="actions">
                                ...
                                <div class="portlet-body">
                                    <?php if (!empty($test_report_percent)) { ?>
                                        <table class="table table-striped table-bordered table-hover masterTable"
                                            id="percentage_report">
                                            <thead>
                                                <tr>
                                                    <th style="text-align:center;">Sr. No.</th>
                                                    <th style="text-align:center;">Question</th>
                                                    <th style="text-align:center; color: green;">Correct<br> Answers (%)</th>
                                                    <th style="text-align:center; color:#F3565D;">Wrong<br> Answers (%)</th>
                                                    <th style="text-align:center; color:#89C4F4;">N/A (%)</th>
                                                    <th style="text-align: center;color:#333;">Options</th>
                                                    <th Style="text-align:center;color:#333;">Correct Answer</th>
                                                    <th Style="text-align:center;color:#333;">Explanation</th>

                                                </tr>
                                            </thead>
                                            <tbody>
                                                <?php $i = 1;
                                                foreach ($test_report_percent as $key) {
                                                    $options_data = $this->percent_model->get_question_options($key->question_id);
                                                    $correct_pct = round(($key->correct_count * 100) / max(1, $key->total_users), 2);
                                                    $wrong_pct = round(($key->wrong_count * 100) / max(1, $key->total_users), 2);
                                                    $na_pct = round(($key->na_count * 100) / max(1, $key->total_users), 2); ?>

                                                    <tr class="odd gradeX">
                                                        <td style="text-align:center;"><?php echo $i++; ?></td>
                                                        <td><?php echo !empty($key->question) ? $key->question : '-'; ?></td>

                                                        <td style="text-align:center;"><?= $correct_pct ?>%</td>
                                                        <td style="text-align:center;"><?= $wrong_pct ?>%</td>
                                                        <td style="text-align:center;"><?= $na_pct ?>%</td>
                                                        <td>
                                                            <ol style="list-style-type: none; padding-left: 0;">
                                                                <?php
                                                                $count = 1;
                                                                foreach ($key->options_data as $option_obj) {
                                                                    echo "<li>" . $count++ . ") " . htmlspecialchars($option_obj->option) . "</li>";
                                                                }
                                                                ?>
                                                            </ol>

                                                        </td>
                                                        <td><?php echo !empty($key->question_mar) ? $key->question_mar : '-'; ?>
                                                        </td>
                                                        <td><?php echo !empty($key->explanation) ? $key->explanation : '-'; ?>
                                                        </td>
                                                    </tr>
                                                <?php } ?>
                                            </tbody>
...
<script>
    function filterExam() {
        ...
        if (test_id) {
            ...
            var baseUrl = "<?php echo base_url('percent_result/percentage_exam_result'); ?>";
...
<script>
    function exportToExcel() {
        ...
        window.location.href = "<?php echo base_url('export_percentage_report'); ?>/" + test_id;
