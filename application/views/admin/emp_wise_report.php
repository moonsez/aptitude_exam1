...
<body>
		...
		<div class="page-content page_div">
			<div class="container">
									...
									<div class="row ">
										...
										<div class="col-md-4">
											<div class="form-group">
												...
												<div class="input-icon">
													<select class="form-control select2me" id="test_name"
														...
														if (!empty($exam_list)) {
															foreach ($exam_list as $key) { ?>
																<option value="<?php echo $key->test_configuration_id; ?>"
																	<?= (!empty($test_id) && $test_id == $key->test_configuration_id) ? 'selected' : ''; ?>>
										...
										<div class="col-md-8 mt-3">
											<button type="button"
												class="btn btn-primary w-100 emp_wise_report">Search</button>
										</div>
									</div>
				...
				<div id="fetchdetailsdiv">
					<?php $this->load->view('admin/emp_wise_report_table'); ?>
				</div>
	...
	<script>
		$(document).on('click', '.emp_wise_report', function () {
			var emp_name = $('#emp_name').val();
			var test_name = $('#test_name').val();
			$('#fullPageLoader').fadeIn();

			$.ajax({
				url: completeURL('percent_result/fetch_empwise_report'),
				type: 'POST',
				dataType: 'json',
				data: {
					emp_name: emp_name,
